function buildTriggerGuidance(triggerType, botName) {
  switch (triggerType) {
    ...
    case 'nickname':
      return `- Trigger type is nickname. The message matched your name or nickname, which is a signal you may be relevant to the conversation. It is not a guarantee that the speaker wants a reply from you. Judge from the wording and recent context whether replying would feel natural.`;
    case 'comment':
...
class ActionPlanner {
  ...
  async plan(sessionId, botName, recentHistory, triggerMessage, options = {}) {
    ...
    const prompt = isIdleCheck
      ...
      : `It is ${new Date().toLocaleString('zh-CN')}. Your name is ${botName}.\n\nRecent group chat:\n${historyText || '(no chat history)'}\n\nTrigger type: ${triggerType}\nTrigger message: ${triggerMessage}\n\nDecide whether this is worth replying to. Prefer silence when you truly have nothing useful to add, but do reply when the message naturally invites your participation, asks for help, greets you, teases you, or you can add clear social value.\n\nDecision rules:\n${triggerGuidance}\n- Questions, greetings, requests, and playful teasing aimed at you should usually be answered with reply.\n- Use wait only when the message is clearly not for you, is pure noise/spam, or a reply would be awkward and add no value.\n- If the message is addressed to you, do NOT classify it as general group chatter.\n\nOutput ONLY valid JSON in exactly this shape: {"action":"reply|wait|complete","reason":"short reason","wait_seconds":0}`;
