...
function getSegmentImageTag(item) {
  const summary = String(item?.summary || item?.data?.summary || '').toLowerCase();
  const subType = String(item?.sub_type ?? item?.data?.sub_type ?? '');
  if (summary.includes('动画表情') || summary.includes('表情') || subType === '1') {
    return '[meme]';
  }
  return '[image]';
}
...
export async function getQuotedContent(e) {
  ...
  const imageTags = imageSegs.map((item) => getSegmentImageTag(item));
...
export function extractContent(e, config, nicknames = []) {
  ...
  for (const item of Array.isArray(e.message) ? e.message : []) {
    ...
    if (item.type === 'text' && textValue && textValue.trim()) {
      ...
      imageTags.push(getSegmentImageTag(item));
