...
class OpenAIChatClient {
  ...
  async describeImage(imageUrl, model, hint = '') {
    logger.info(`[crystelf-ai-v2] describeImage request model=${model} hint=${JSON.stringify(hint)} url=${imageUrl}`);
    const response = await this.client.chat.completions.create({
      model,
      temperature: 0.3,
      max_tokens: 600,
    ...
    logger.info(`[crystelf-ai-v2] describeImage result content=${JSON.stringify(description)}`);
    return description;
  }
}
