...
DO NOT include any explanation, markdown formatting, or additional text. Only output the JSON.`;
    ...
    try {
      logger.info(
        `[crystelf-ai-v2] planner prompt session=${sessionId} last=${JSON.stringify(String(triggerText || '').slice(0, 80))} idle=${isIdleCheck}\n${prompt}`
      );
      ...
      if (!content || !content.trim()) {
        logger.warn('[crystelf-ai-v2] planner empty response, fallback reply');
      ...
      if (!jsonStr) {
        logger.warn(
          `[crystelf-ai-v2] planner failed to find json: ${String(content).slice(0, 100)}`
        );
      ...
      try {
        ...
        try {
          ...
          logger.warn(`[crystelf-ai-v2] planner failed to parse json: ${jsonStr.slice(0, 100)}`);
          return { action: 'reply', reason: 'parse failed', rawResponse: content };
        }
      }
      ...
      logger.info(
        `[crystelf-ai-v2] planner final session=${sessionId} action=${action} reason=${JSON.stringify(result.reason)}${result.waitMs ? ` waitMs=${result.waitMs}` : ''}`
      );
      return result;
    } catch (error) {
      logger.error(`[crystelf-ai-v2] planner error: ${error.message}`);
