...
export async function extractGifFrames(gifUrl) {
  try {
    ...
    if (!response.ok) {
      logger.error(`[crystelf-ai-v2] gif fetch failed status=${response.status} url=${gifUrl}`);
      return null;
    }
    ...
    logger.info(
      `[crystelf-ai-v2] gif extracted frames=${frames.length} total=${totalPages} url=${gifUrl}`
    );
    return { frames, buffer };
  } catch (error) {
    logger.error(`[crystelf-ai-v2] gif extract failed: ${error.message}`);
    return null;
  }
}
