class TopicTracker {
  constructor(ai, config, db) {
    ...
    this.config = config;
    this.db = db;
    this.lastAnalyzedWindowEnd = new Map();
  }

  async onMessage(sessionId) {
    if (!this.config.topic?.enabled) return;
    if (!String(sessionId || '').startsWith('group:')) return;
  ...
  getTopicContext(sessionId, historyStartAt) {
    ...
    for (let index = 0; index < windowCount; index++) {
      const windowEndAt = firstWindowEnd - index * windowMs;
      ...
      const keywordsLine = keywords.length > 0 ? ` | 关键词: ${keywords.join(', ')}` : '';
      lines.push(`- ${timeRange}: ${String(topic.summary).trim()}${keywordsLine}`);
    }
  ...
  async analyzeWindow(sessionId, windowStartAt, windowEndAt) {
...
Rules:
1. Use ONLY messages from this log. Do not infer anything outside this time window.
...
Output strictly in JSON format:
{
  ...
  "summary": "one concise abstract summary sentence"
    ...
    const title = String(parsed.title || '').trim() || `${this.formatTime(windowStartAt)} 话题概览`;
    ...
    logger.info(
      `[crystelf-ai] topic summarized session=${sessionId} window=${this.formatTime(windowStartAt)}~${this.formatTime(windowEndAt)} count=${userMessages.length}`
    );
  }
  ...
  parseKeywords(raw) {
    try {
      return this.normalizeKeywords(JSON.parse(raw));
    } catch {
      return [];
    }
  }
