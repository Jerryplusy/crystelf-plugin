...
function migrateLegacyAiConfig(legacyConfig, defaultAiConfig) {
  ...
  if (legacyConfig.topic?.windowHours === undefined) {
    delete migrated.topic.messageThreshold;
    delete migrated.topic.timeThresholdMs;
    delete migrated.topic.maxTopicsPerSession;
  }
  ...
  if (legacyConfig.expression?.learnAfterMessages === undefined) {
    delete migrated.expression.maxExpressions;
  }
...
function sanitizeConfig(name, data) {
  if (name !== 'ai' || !isObject(data)) return data;

  const next = deepClone(data);
  delete next.maxIterations;
  delete next.toolCallConstraintStrength;
  delete next.memory;

  if (isObject(next.topic)) {
    delete next.topic.messageThreshold;
    delete next.topic.timeThresholdMs;
    delete next.topic.maxTopicsPerSession;
  }

  if (isObject(next.expression)) {
    delete next.expression.maxExpressions;
  }
...
async function readPluginConfigFiles() {
  try {
    return (await fsp.readdir(pluginConfigPath)).filter((file) => file.endsWith('.json'));
  } catch (error) {
...
async function init() {
  try {
    ...
    const pluginFiles = await readPluginConfigFiles();
    ...
    const files = (await fsp.readdir(dataConfigPath)).filter((file) => pluginFileSet.has(file));
    configCache = {};
...
function watchConfigs() {
  ...
  readPluginConfigFiles().then((pluginFiles) => {
    const pluginFileSet = new Set(pluginFiles);
    return fsp.readdir(dataConfigPath).then((files) => ({ files, pluginFileSet }));
  }).then(({ files, pluginFileSet }) => {
    files
      .filter((file) => pluginFileSet.has(file))
      .forEach((file) => {
        const filePath = path.join(dataConfigPath, file);
