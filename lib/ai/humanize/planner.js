...
class ActionPlanner {
  ...
  async plan(sessionId, botName, recentHistory, lastTriggerMessage, options = {}) {
    ...
    if (isIdleCheck) {
      const staticPrefix = `IMPORTANT: There is no specific trigger message this time. You are being proactively invited to observe the group chat and decide whether to speak up.
...
DO NOT include any explanation, markdown formatting, or additional text. Only output the JSON.`;
      const dynamicSuffix = `It is ${timeStr}. Your name is ${botName}.

Here is the recent chat content (from oldest to most recent):
${chatBlock || '(no chat history)'}`;
      prompt = this.config.enablePromptCacheOptimization === true
        ? `${staticPrefix}\n\n--- Dynamic information below ---\n\n${dynamicSuffix}`
        : `${dynamicSuffix}\n\n${staticPrefix}`;
    } else {
      const staticPrefix = `Available actions:
