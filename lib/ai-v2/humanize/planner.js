class ActionPlanner {
  ...
  async plan(sessionId, botName, recentHistory, triggerMessage, isIdleCheck = false) {
    ...
    try {
      ...
      if (!match) {
        return { action: 'reply', reason: 'planner parse failed' };
      }
      ...
      return { action: 'reply', reason: 'planner error' };
    }
  }
}
