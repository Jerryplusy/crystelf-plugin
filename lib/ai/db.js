...
class JsonChatDatabase {
  ...
  getMessagesByTimeRange(sessionId, startTimestamp, endTimestamp) {
    this.init();
    const rows = this.data.messages
      .filter(
        (row) =>
  ...
  getTopicByWindow(sessionId, windowStartAt, windowEndAt) {
    this.init();
    const row = this.data.topics
      .filter(
        (item) =>
          item.sessionId === sessionId &&
          item.windowStartAt === windowStartAt &&
          item.windowEndAt === windowEndAt
      )
  ...
  getExpressionsByUser(userId, limit = 50) {
    this.init();
    const rows = this.data.expressions
  ...
  replaceExpressionsByUser(userId, userName, expressions) {
    this.init();
    this.data.expressions = this.data.expressions.filter(
