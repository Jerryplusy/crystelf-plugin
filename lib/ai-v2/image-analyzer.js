...
export async function calculateImageHash(url) {
  try {
    ...
    logger.warn(`[crystelf-ai-v2] image hash fallback to url: ${error.message}`);
    return crypto.createHash('md5').update(url).digest('hex');
  }
}
...
async function downloadImage(url, savePath) {
  return await new Promise((resolve) => {
    ...
    protocol
      ...
      .on('error', (error) => {
        unlink(savePath, () => {});
        logger.error(`[crystelf-ai-v2] image download failed: ${error.message}`);
        resolve(false);
      });
  });
}
...
export async function analyzeImage(ai, imageUrl, model, gifBuffer) {
  try {
    logger.info(`[crystelf-ai-v2] image analyze request model=${model} url=${imageUrl}`);
...
Response format (JSON):
{
  ...
  "characters": ["character1", "character2"]
    ...
    try {
      ...
      logger.warn(
        `[crystelf-ai-v2] image analyze fallback without response_format model=${model} reason=${message}`
      );
      response = await ai.client.chat.completions.create(requestPayload);
    }

    const raw = response.choices?.[0]?.message?.content || '';
    logger.info(`[crystelf-ai-v2] image analyze raw url=${imageUrl} content=${JSON.stringify(raw)}`);
    ...
    logger.info(
      `[crystelf-ai-v2] image analyze result type=${result.type} emotion=${result.emotion || ''} characters=${JSON.stringify(result.characters || [])} description=${JSON.stringify(result.description || '')}`
    );
    return result;
  } catch (error) {
    logger.error(`[crystelf-ai-v2] image analyze failed: ${error.message}`);
...
export async function processImage(ai, imageUrl, model, db) {
  try {
    ...
    if (existing) {
      logger.info(`[crystelf-ai-v2] image exists hash=${hash} description=${existing.description}`);
      return existing;
    }

    const analysis = await analyzeImage(ai, imageUrl, model);
    if (!analysis.success || !analysis.type) {
      logger.warn(`[crystelf-ai-v2] image analysis failed url=${imageUrl} error=${analysis.error || ''}`);
      return null;
    }
        ...
        characters.map(async (character) => {
          ...
          if (analysis.gifBuffer) {
            try {
              ...
              logger.info(`[crystelf-ai-v2] meme saved character=${character} path=${targetPath}`);
              return targetPath;
            } catch (error) {
              logger.warn(`[crystelf-ai-v2] meme save failed character=${character}: ${error.message}`);
              return null;
            }
          }
          const downloaded = await downloadImage(imageUrl, targetPath);
          if (downloaded) {
            logger.info(`[crystelf-ai-v2] meme downloaded character=${character} path=${targetPath}`);
            return targetPath;
          }
          logger.warn(`[crystelf-ai-v2] meme download failed character=${character} url=${imageUrl}`);
          return null;
        })
      );
    ...
    db.saveImage(record);
    logger.info(`[crystelf-ai-v2] image record saved hash=${hash} type=${record.type} description=${record.description}`);
    return record;
  } catch (error) {
    logger.error(`[crystelf-ai-v2] processImage failed: ${error.message}`);
    return null;
  }
}
