class TopicTracker {
  ...
  async analyzeTopics(sessionId) {
    ...
    logger.info(`[crystelf-ai-v2] topic raw session=${sessionId} content=${JSON.stringify(response)}`);
    ...
    logger.info(`[crystelf-ai-v2] topic selected session=${sessionId} count=${Math.min(parsed.topics.length, 3)} titles=${JSON.stringify(parsed.topics.slice(0, 3).map((item) => item.title || ''))}`);
  }
