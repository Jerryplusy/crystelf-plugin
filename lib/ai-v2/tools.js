...
function createInfoTools(toolCtx) {
  ...
  if (toolCtx.config.isMultimodal) {
    tools.push({
      ...
      handler: async (args) => {
        if (imageAnalysisBlocked) {
          logger.info(`[crystelf-ai-v2] view_image blocked user=${toolCtx.userId} group=${toolCtx.groupId}`);
        ...
        try {
          ...
          logger.info(`[crystelf-ai-v2] view_image result message=${args.message_id} content=${JSON.stringify(description)}`);
    ...
    tools.push({
      ...
      handler: async (args) => {
        if (imageAnalysisBlocked) {
          logger.info(`[crystelf-ai-v2] view_member_avatar blocked user=${toolCtx.userId} group=${toolCtx.groupId}`);
        ...
        try {
          ...
          logger.info(`[crystelf-ai-v2] view_member_avatar result user=${args.user_id} content=${JSON.stringify(description)}`);
          return { success: true, description };
        } catch (error) {
